...
const TestProvider = React.memo(({ children }) => {
  ...
  const [timer, setTimer] = useState(localStorage.getItem("time") || 30);
  ...
  const [activeSubTool, setActiveSubTool] = useState(
    localStorage.getItem("activeSubTool") || 15
  );
  ...
  useEffect(() => {
    const storedLang = localStorage && localStorage.getItem("lang");
    const storedTime = localStorage && localStorage.getItem("time");
    const storedActiveTool = localStorage && localStorage.getItem("activeTool");
    const storedActiveSubTool = localStorage && localStorage.getItem("activeSubTool");
  
    if (storedLang) setLang(storedLang);
    if (storedTime) setTime(parseInt(storedTime, 10));
    if (storedActiveTool) setActiveTool(storedActiveTool);
    if (storedActiveSubTool) setActiveSubTool(parseInt(storedActiveSubTool, 10));
  }, []);
